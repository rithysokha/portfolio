<script setup>
import profileImage from '../../assets/pictures/profile.webp'
</script>
<template>
  <main class="bg-[#f9f9f9] sm:flex sm:py-40">

    <div class="w-fit">
      <div class="flex items-center sm:justify-end">
        <div class="w-[100vw] sm:w-[40vw] flex items-center justify-center space-x-10 sm:flex-col sm:space-x-0 2xl:items-end">
          <div>
          <img class="w-[40vw] sm:w-[20vw] 2xl:w-[10vw]  border-gray-300" :src="profileImage" alt="Profile" />
          <div class="ml-1">
          <p class="font-bold underline text-l 2xl:text-2xl">Sokha Rithy</p>
          <p class="text-sm 2xl:text-lg">Web Developer</p>
        </div>
      </div>
    </div>
      </div>
    </div>
    <div class=" mx-5 w-[90vw] sm:flex sm:items-center">
        <div class=" text-justify 2xl:w-[35vw] 2xl:ml-6">
        <p class="font-bold lg:text-xl 2xl:text2xl">Bio:</p>
        <p class="lg:text-lg xl:text-xl 2xl:text-2xl">
          <span class="pl-6" /> I am a web developer who is passionate about various web technologies. I
          have experience in developing web applications using React.js, Vue.js and building api with Python
          Flask, Java Spring Boot. I am also familiar with deployment
          using Nginx, Docker, and Linux Server.<br/>
          <span class="pl-4" /> Currently I'm an IT Engineering Student at Royal University of Phnom Penh.
        </p> 
    </div>

</div>
  </main>
</template>
